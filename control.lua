...
function controlMod.postProcessTiming(pkt, now, monitorPipe)
...
function controlMod.postProcessTimingNew(pkt, now)
   -- if this is a packet about to be sent out from source
   -- check for old time in payload
   -- compare with new time and log RTT
   -- if this is a packet about to be sent out from receiver
...
function controlMod.percc1ProcessAndGetRate(pkt)
   ...
   -- controlMod.log("At end host "
   -- 	 .. "\n  bottleneck rates are " .. bnRate1 .. " and " .. bnRate2
   -- 	 .. "\n  maxHops was " .. maxHops
   -- 	 .. "\n  bottleneck links are ..")
   -- for i=1,maxHops do
   --    if bnBitmap[i] == 1 then controlMod.log(i .. " ") end
   -- end
   --   controlMod.log("  setting rates etc. now")
   assert(bnRate1 ~= nil)
   assert(bnRate2 ~= nil)
   assert(bnBitmap ~= nil)
...
function controlMod.controlSlave(dev, pipes, readyInfo, monitorPipe)
	 ...
	 perc_constants.NEW_FLOWS_PER_CONTROL_LOOP)
	  ...
	  do
		...
		if pkt.percc1:getIsForward() == percc1.IS_FORWARD then
		   ...
		   if pkt.percc1:getIsExit() == percc1.IS_EXIT then -- receiver echoed fin
		      controlMod.log("tx control gets exit pkt for flow " .. pkt.percg:getFlowId())
		      controlMod.log("Flow " .. flowId
		   ...
		   elseif queues[flowId] == nil then -- flow ended
		      controlMod.log("tx control gets regular pkt for no-more-data flow " .. pkt.percg:getFlowId())
		      controlMod.percc1ProcessAndGetRate(pkt)
		      controlMod.postProcessTiming(pkt, now, monitorPipe)
		      ...
		      controlMod.log("tx control gets regular pkt for have-more-data flow " .. pkt.percg:getFlowId())
			 ...
			 monitorPipe:send(
		      ...
		      controlMod.log("flow was assigned queue " .. queueNo)
		      assert(queueNo ~= nil)
		      local rateInfo = queueRates[queueNo]
			 ...
			 if rate1 < rateInfo.currentRate then
			    ...
			    if rate1 > perc_constants.END_HOST_LINK_MBPS then
			       rateInfo.currentRate = 500
			       configuredRate = 500
			       controlMod.warn("received bottleneck rate > link cap, set to 500 Mb/s")
			       end
			    ...
			    controlMod.log("  new rate " .. rate1 .. " is bigger than current " .. rateInfo.currentRate)
			    ...
			    elseif rateInfo.nextRate == rate1 then
			       controlMod.log("     next rate is the same as new rate, do nothing")
	  ...
	  do -- MAKES NEW PACKETS
		...
		noNewPackets = 0
		controlMod.log("make  new packets")
		local numNew = 0
		for msgNo, msg in pairs(msgs) do
		   -- controlMod.log("msg.flow " .. msg.flow .. ", msg.size " .. msg.size .. ", msg.destination " .. msg.destination)
		   ...
		   if next(freeQueues) ~= nil then 
			 ...
			 monitorPipe:send(
		      ...
		      controlMod.log("assigned a rateInfo struct for " .. flowId)
		      ...
		      numNew = numNew + 1
		      -- tell data thread
		      controlMod.log("telling data thread to start flow"
					 .. msg.flow .. " of size "
					 .. msg.size .. " packets "
					 .." from queue " .. queue)
		      ...
		      controlMod.postProcessTimingNew(pkt, now)
		...
		controlMod.log("Sent " .. numNew .. " new packets")
	     else
		noNewPackets = noNewPackets + 1
		if (noNewPackets % 100000 == 0) then
		   controlMod.log("No new packets: " .. noNewPackets .. " at time " .. dpdk.getTime())
		end
	     end -- if msgs ~= nil
	  end
	  ...
	  do -- DEALLOCATE QUEUES
		...
		ipc.fastAcceptMsgs(
	     ...
	     if next(msgs) ~= nil then
		controlMod.log("deallocate queues from completed flows")
		for msgNo, msg in pairs(msgs) do
		   local flowId = msg.flow
		   assert(flowId >= 100)
		   local queueNo = queues[flowId]
		   ...
		   if queueNo ~= nil then
			 ...
			 monitorPipe:send(
		      ...
		      controlMod.warn("Got a FINACK message from data for flow " .. flowId .. " without an active queue.\n")
		   end
		end
	     end
	  ...
	  do -- CHANGE RATES
		...
		and queueRates[queueNo].changeTime <= now then
		   controlMod.log("change rates for flow " .. flowId
				      .. ", queue " .. queueNo
				      .. " from " ..
		   ...
		   if configuredRate > perc_constants.END_HOST_LINK_MBPS then
		      queueRates[queueNo].currentRate = 500
		      configuredRate = 500
		      controlMod.warn("had received bottleneck rate > link cap, set to 500 Mb/s")
		   end
		   local dTxQueue = dev:getTxQueue(queueNo)
		   dTxQueue:setRate(configuredRate)
